...
// @Summary Get Sum Order
// @Description Get Sum Order
// @Tags Order
// @Accept json
// @Produce json
// @Param order body models.GetSumRequest true "GetSumRequest"
// @Success 201 {object} Response{data=string} "Success Request"
// @Response 400 {object} Response{data=string} "Bad Request"
// @Failure 500 {object} Response{data=string} "Server Error"
func (h *Handler) GetOrderSum(c *gin.Context) {
	var getSum models.GetSumRequest

	err := c.ShouldBindJSON(&getSum)
	if err != nil {
		h.handlerResponse(c, "get sum order", http.StatusBadRequest, err.Error())
		return
	}

	resp, err := h.storages.Order().GetSumOrder(context.Background(), &getSum)
	if err != nil {
		h.handlerResponse(c, "get sum order storage", http.StatusBadRequest, err.Error())
		return
	}
	h.handlerResponse(c, "get sum order", http.StatusCreated, resp)
}
