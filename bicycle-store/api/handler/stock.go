...
// @Router /transfer/quantity [POST]
// @Summary Transfer Quantity
// @Description Transfer Quantity
// @Tags Stock
// @Accept json
// @Produce json
// @Param stock body models.TransferQuantityRequest true "TransferQuantityRequest"
// @Success 202 {object} Response{data=string} "Success Request"
// @Response 400 {object} Response{data=string} "Bad Request"
// @Failure 500 {object} Response{data=string} "Server Error"
func (h *Handler) TransferQuantity(c *gin.Context) {
	var updateStock models.TransferQuantityRequest

	err := c.ShouldBindJSON(&updateStock)
	if err != nil {
		h.handlerResponse(c, "transfer quantity", http.StatusInternalServerError, err.Error())
		return
	}

	err = h.storages.Stock().UpdateQuantity(context.Background(), &updateStock)
	if err != nil {
		h.handlerResponse(c, "storage update quantity error ", http.StatusInternalServerError, err.Error())
		return
	}
	h.handlerResponse(c, "update stock", http.StatusAccepted, "resp")
}
