...
// UPDATE Staff godoc
// @ID get_list_staff_with_date
// @Router /get-list/staff [GET]
// @Summary Get List Staff With Date
// @Description Get List Staff With Date
// @Tags Staff
// @Accept json
// @Produce json
...
// @Response 400 {object} Response{data=string} "Bad Request"
// @Failure 500 {object} Response{data=string} "Server Error"
func (h *Handler) GetListStaffWithDate(c *gin.Context) {

	date := c.Query("time")

	fmt.Println(">>>>>>",date)

	resp, err := h.storages.Staff().GetListStaffWithDate(context.Background(), &models.Date{OrderDate: date})
	if err != nil {
