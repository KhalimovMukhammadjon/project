...
func (r *orderRepo) GetSumOrder(ctx context.Context, req *models.GetSumRequest) (*models.GetSumResponse, error) {
	var (
		discountPrice, orderLimitPrice int
		orderSum, totalSum             float64
	)
	...
	if req.CouponName != "" {
		query := `
			...
			WHERE name =$1
		`
		err := r.db.QueryRow(ctx, query, req.CouponName).Scan(
			&orderLimitPrice,
			&discountPrice,
		)
		if err != nil {
			return &models.GetSumResponse{}, err
		}
	}

	query := `
			SELECT
				list_price * CAST(quantity AS NUMERIC) as sum
	...
	totalSum = orderSum - float64(discountPrice)

	return &models.GetSumResponse{
